document.addEventListener('DOMContentLoaded', function () {
		...
		estadoForm.addEventListener('submit', function (event) {
			...
			fetch('/guardar_cambios/', {
				...
				.then(data => {
					...
					const habitacionElement = document.querySelector(
						`.estado[data-habitacion-id="${data.id_bedroom}"]`
					)
	...
	// Eliminar habitación
	const botonesEliminarHabitacion = document.querySelectorAll(
		'.btn-eliminar-habitacion'
	)
	...
	function filtrarHabitaciones() {
		...
		for (let i = 0; i < habitaciones.length; i++) {
			...
			const nombreHabitacion = habitacion
